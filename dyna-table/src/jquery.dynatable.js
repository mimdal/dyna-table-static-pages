  ...
  $.dynatable = function(element, options) {
    ...
    var defaults = {
          ...
          inputs: {
            ...
            perPageText: 'page size: ',
            recordCountText: 'show items ',
    ...
    plugin.recordCount = {
      create: function() {
        ...
        if (recordsShown < recordsQueryCount && settings.features.paginate) {
          ...
          text += "<span class='dynatable-record-bounds'>" + (bounds[0] + 1) + " to " + bounds[1] + "</span> total  ";
        } else if (recordsShown === recordsQueryCount && settings.features.paginate) {
          text += recordsShown + " total ";
        }
        text += recordsQueryCount + " " + collection_name;
        if (recordsQueryCount < recordsTotal) {
          text += " (total number " + recordsTotal + " item)";
        }
